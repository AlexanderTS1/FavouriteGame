...
def ReflectionY(vertices):
	...
	result = []
	for item in vertices:
		point = np.dot(Ry, item)
		result.append(point)
	return result

def DeformationX(vertices, shx):
	Dx = [[1, shx, 0], [0, 1, 0], [0, 0, 1]]
	result = []
	for item in vertices:
		point = np.dot(Dx, item)
		result.append(point)
	return result

def DeformationY(vertices, shy):
	Dy = [[1, 0, 0], [shy, 1, 0], [0, 0, 1]]
	result = []
	for item in vertices:
		point = np.dot(Dy, item)
		result.append(point)
	return result
